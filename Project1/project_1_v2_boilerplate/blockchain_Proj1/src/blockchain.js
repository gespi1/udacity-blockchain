...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let diffTime = currentTime - msgTime
            if ( diffTime <= 300 ) {                            
                if (!bitcoinMessage.verify(message, address, signature)) {
                    reject('bitcoin message wasnt able to be verified')
                }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (let i = self.chain.height; i < 0; i--) {
                if (i == 0) {
                    errorLog.push("gensisblock has no previous hash")
                } else if (!await self.chain[i].validate()) {
                    errorLog.push(`invalid block ${self.chain[i].hash}`)
                } else if (self.chain[i].previousBlockHash != self.chain[i-1].hash) {
                    errorLog.push(`${self.chain[i-1].hash} doesnt match this blocks previous hash ${self.chain[i].previousBlockHash}`)
                }
            }
            resolve(errorLog)
        });
